...
use crate::{state::*, MAXIMUM_AGE, SOL_USD_FEED_ID, USDC_USD_FEED_ID};
...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()>{
   ...
   match ctx.accounts.collateral_mint.key() {
    key if key == user.usdc_address => {
        let new_usdc = calculate_accrued_interest(user.deposited_usdc, collateral_bank.interest_rate, user.last_updated_deposited)?;
        total_collateral = new_usdc * usdc_price.price as u64;
        let new_sol = calculate_accrued_interest(user.borrowed_sol, collateral_bank.interest_rate, user.last_updated_borrowed)?;
        total_borrowed = new_sol * sol_price.price as u64;
    },
    _ => {
        let new_sol = calculate_accrued_interest(user.deposited_sol, collateral_bank.interest_rate, user.last_updated_deposited)?;
        total_collateral = new_sol * sol_price.price as u64;
        let new_usdc = calculate_accrued_interest(user.borrowed_usdc, collateral_bank.interest_rate, user.last_updated_borrowed)?;
